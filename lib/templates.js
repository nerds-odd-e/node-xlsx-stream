...
module.exports = {
  worksheet: {
    header: function(opts) {
      ...
      if (options.columnsWidth) {
        ...
        for (colIdx in _ref1) {
          width = _ref1[colIdx];
          colsTag += "<col min=\"" + colIdx + "\" max=\"" + colIdx + "\" width=\"" + width + "\" customWidth=\"1\" />";
        }
      }
      cols = colsTag ? "  <cols>\n    " + colsTag + "\n  </cols>" : "";
  ...
  sheet_related: {
    ...
    "xl/workbook.xml": {
      ...
      sheet: function(sheet) {
        return xml("<sheet name=\"" + (esc(sheet.name)) + "\" sheetId=\"" + (esc(sheet.index)) + "\" state=\"" + (sheet.opts && sheet.opts.hideSheet ? "hidden" : "visible") + "\" r:id=\"rSheet" + (esc(sheet.index)) + "\"/>");
      },
